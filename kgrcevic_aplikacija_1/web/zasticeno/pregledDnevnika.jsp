...
<html>
    <head>
        ...
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
            <title>Pregled korisničkih zahtjeva vidljivog dijela aplikacije</title>        
                            ...
                            <form method="GET" name="meteo">
                                <div class="row-fluid">
                                    <table style="width: 750px;">
                                        <th colspan="2" style="text-decoration:underline">Pregled korisničkih zahtjeva vidljivog dijela aplikacije</th>
                                        <br/>
                                            ...
                                            <td>
                                                ...
                                                <input type="radio" id="radio2" name="vrsta" value="1">
                                                    ...
                                                    <input type="radio" id="radio3" name="vrsta" value="2">
                                                        ...
                                                        <tr><td colspan="2"><center><input type="submit" style="opacity:0.5" value="Prikaži" class="btn btn-large btn-block"/></center></td></tr>
                                                        </table>
                                                        </div>
                                                        </form>
                                                            ...
                                                            <c:if test="${param.redaka!=null}">
                                                                <c:choose>
                                                                    <c:when test="${param.vrsta=='1'}">
                                                                        <sql:query dataSource="${con}" var="podaci">SELECT * from kgrcevic_dnevnik where vrijemepocetka BETWEEN concat(substring('<c:out value="${param.datumOD}"/>',1,13),':',substring('<c:out value="${param.datumOD}"/>',15,2),':',substring('<c:out value="${param.datumOD}"/>',18,2)) AND concat(substring('<c:out value="${param.datumDO}"/>',1,13),':',substring('<c:out value="${param.datumDO}"/>',15,2),':',substring('<c:out value="${param.datumDO}"/>',18,2))</sql:query>
                                                                    </c:when>
                                                                    <c:when test="${param.vrsta=='2'}">                       
                                                                        <sql:query dataSource="${con}" var="podaci">SELECT * from kgrcevic_dnevnik</sql:query>
                                                                    </c:when>
                                                                </c:choose>

                                                                <display:table name="${podaci.rows}" pagesize="${param.redaka}" style="border:1px solid #666; text-align:center" >   
                                                                    <display:column property="zahtjev" title="Zahtjev korisnika" style="border:1px solid #666; width:350px; text-align:left"/>
                                                                    <display:column property="vrijemepocetka" title="Vrijeme početka zahtjeva" style="border:1px solid #666; width:250px"/>
                                                                    <display:column property="vrijemezavrsetka" title="Vrijeme završetka zahtjeva" style="border:1px solid #666; width:250px"/>   
                                                                    <display:column property="trajanje" title="Trajanje(ms)" style="border:1px solid #666; width:100px"/>
                                                                </display:table>       
                                                            </c:if>     
                                                        </c:if>     
                                                        </center>
